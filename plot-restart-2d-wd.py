#!/usr/bin/env python
# plot variables at restart file's lowest layers with surface wind 
...
lines=sorted(glob.glob(ainput['filelist']))
nlines=len(lines)
stime={}
n=0
for aline in lines:
...
if barb_yn:
  metfiles=sorted(glob.glob(ainput['metfiles']))
  mlines=len(metfiles)
  mettime={}
  m=0
  for aline in metfiles:
    itmp=aline.rindex('.fv_srf_wnd')
    tstring1=aline[itmp-15:itmp]
    mettime[m]=datetime.strptime(tstring1,'%Y%m%d.%H%M%S')
    if mettime[m] != stime[m]:
     print('inconsistent mettime and tracer time',m,aline,mettime[m],stime[m])
     sys.exit(1)
    m=m+1
...
while ftstart <= ftend:
  ...
  if barb_yn:
    fmet=xr.open_dataset(metfiles[n])
    if im != fmet['xaxis_1'].size or jm != fmet['yaxis_1'].size: 
       print('inconsistent surf wind file xaxis or yaxis')
       sys.exit(1)
...
# variable loop
  for m in range(len(modnames)):
   ...
   if barb_yn :
      ax.barbs(geolon[::barb_skip,::barb_skip].values,geolat[::barb_skip,::barb_skip].values,
         fmet['u_srf'][0,::barb_skip,::barb_skip].values.squeeze(),
         fmet['v_srf'][0,::barb_skip,::barb_skip].values.squeeze(),
